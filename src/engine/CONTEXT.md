...
- **Engine ↔ Web**: Web calls the Engine via `POST /move` (FEN + adapter_id + recent_positions → Move) and `WebSocket /play`. Engine returns one legal Move per call. Web owns game-state *storage* (the source of truth for the running Game lives in the browser's `chess.js`) but attaches the relevant history slice to every request so the Engine's history-aware legality mask works without server-side session state. Terminal-state detection and draw claiming remain Web-side; the Engine is never asked "is the Game over?", only "given this Position and history, what is your Move?".
