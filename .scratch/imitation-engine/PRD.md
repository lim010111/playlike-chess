...
- **Per-Player opponent filter (Q-rating-gap)**: only `rated == true` games where the opponent's post-game rating is within ±400 Elo of the Player's own post-game rating (per the Chess.com PubAPI per-game `rating` snapshot — documented as "the player's rating after the game finished"). Filter is *relative to the Player's own rating*, not an absolute floor, so it adapts to each Roster member's native pool (Super-GM vs GM). Rationale: super-GM archives mix serious play with low-rated trolling/handicap games whose move distribution is not representative of the Player's style; ±400 keeps the competitive band (Elo formula → opponent expected score 9–91%) without leaning on a one-size-fits-all threshold.
- **Tokenizer + action space (Q8)**: char-level FEN tokenization on the **canonicalized 4-field FEN** (`<piece placement> <side> <castling> <ep>` — halfmove clock and fullmove number are deliberately *dropped* before tokenization). Vocab ~32, fixed length sufficient to fit the bounded canonical FEN (target ~80 with padding). UCI 1968 actions for the policy head; legality mask provided by `python-chess`. Rationale for dropping the two clock fields: (a) FIDE's "same Position" definition for threefold also ignores them, so this aligns model input with the equivalence semantics used by the history-aware legality stage; (b) fullmove number is a weak phase signal already encoded better in piece placement; (c) halfmove clock has a real but niche use case (50-move-rule endgame holding strategy) that 9M params is unlikely to learn at v1 scale, and dropping it gives a cleaner baseline for v2 ablation if endgame qualitative review shows the gap. Revisit trigger: qualitative review flags incompetent play in halfmove ≥ 80 positions → consider adding halfmove clock back as model input in v2.
...
2. **Chess.com Player Ingester** — input: Chess.com username, time-control filter, rating-gap threshold; output: shards of `(FEN, played-move)` Positions for that Player. Encapsulates PubAPI archive paging, rate-limit-aware retry/backoff, JSON-to-PGN normalization, opponent-rating-gap filter (default ±400 Elo against the Player's own post-game rating, using the PubAPI's per-game `rating` snapshot), `rated == true` filter, Position extraction.
3. **FEN Tokenizer** — interface: `fen → int tensor`, `int tensor → canonical_fen`. The encoder accepts any standard FEN and *canonicalizes* it by stripping the halfmove clock and fullmove number, then tokenizes the resulting 4-field string (`<piece placement> <side> <castling> <ep>`). The decoder produces a canonical 4-field FEN. Round-trip is over the canonical form: `decode(encode(fen)) == canonicalize(fen)`. Char-level vocabulary with padding and a fixed sequence length sized for the bounded canonical FEN. The canonical form is what threefold-repetition equality cares about and what the model needs — see PRD Q8 for the rationale on dropping the two clock fields.
4. **Action Vocabulary** — interface: `uci_string ↔ index`, `legal_mask(board, move_history=None) → bool[1968]`. UCI move enumeration, index assignment stable across runs, mask alignment validated against `python-chess`. When `move_history` (UCI moves from the Game's first move) is supplied, the mask additionally excludes Moves that would cause threefold repetition — implemented by replaying the history on a `python-chess` `Board` and calling `board.is_repetition(3)` after each candidate push (history-aware stage; see `src/engine/CONTEXT.md`).
...
11. **Web API** — FastAPI routes: `POST /move` (FEN + adapter_id + move_history → Move), `WebSocket /play` (Session-state turns — see `src/web/CONTEXT.md` for the Session vocabulary), `GET /roster` (list of available Adapters).
...
- `POST /move` — body `{ fen: string, adapter_id: "magnus" | "hikaru" | "firouzja" | "caruana" | "sindarov", decoding: "greedy" | "sample", temperature?: number, move_history?: string[] }` → response `{ move_uci: string, policy?: { uci: string, prob: number }[] }`. `move_history` is the full UCI move list from the Game's first move (e.g. `["e2e4", "e7e5", ...]`), used by the Engine's history-aware legality stage to mask threefold-repetition-causing Moves. The Engine replays the moves on a `python-chess` `Board` and uses `board.is_repetition(3)` for the check — delegating FIDE's "same Position" semantics (ep target, castling rights, halfmove clock handling) to the library rather than re-implementing it. If `move_history` is omitted, only rule-legality applies. Engine guarantees the returned Move is legal in the given Position.
...
- **Multiplayer / matchmaking / accounts**. v1 is one-Session-per-browser-tab (one human, one chosen Opponent — see `src/web/CONTEXT.md`) with no persistence beyond the active Session state in the browser tab. Multiple independent browser Sessions are supported and processed serially through the single GPU; the practical concurrent-user ceiling is documented in `src/engine/CONTEXT.md`.
...
- `CONTEXT-MAP.md` at the repository root and three per-context CONTEXT files establish the domain vocabulary used throughout this PRD: `src/training/CONTEXT.md` (Position, Game, Player, Roster, Base model, Adapter, Move-match accuracy, Ladder bracket, Head-to-head Elo delta), `src/engine/CONTEXT.md` (Inference Engine, Decoding mode, Legality mask, Policy distribution), and `src/web/CONTEXT.md` (Session, Opponent, Terminal state). User-story prose uses colloquial "game" where the natural-language sense is meant; technical contracts and module specs use the typed term **Session** for the live-play concept to keep it separable from the archived Training **Game**.
