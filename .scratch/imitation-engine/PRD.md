...
5. As a chess player, I want the model's response within a couple of seconds, so that the game feels interactive rather than batch.
...
28. As the project maintainer, I want an Evaluation Harness that runs four checks (top-k Move-match accuracy, Self-play Elo via Fairy-Stockfish ladder, opening repertoire KL divergence, illegal-move count), so that I can decide pass/fail per Adapter without manual inspection.
...
36. As a future contributor, I want a `CONTEXT-MAP.md` plus per-context `CONTEXT.md` files defining vocabulary (Position, Game, Player, Roster, Base model, Adapter, Move-match accuracy, Self-play Elo), so that I can read the codebase without re-deriving terminology.
...
- **Imitation definition (Q2)**: training loss = position-unit top-1 cross-entropy on (FEN, played-move). Evaluation is multi-metric: top-k Move-match accuracy + Self-play Elo + opening repertoire similarity.
...
- **Evaluation thresholds (Q12)**: top-1 ≥ 40%, top-3 ≥ 70%; Base ≥ 1800 Elo on Fairy-Stockfish ladder; Adapter ≥ Base (no strength regression — stretch goal Adapter ≥ Base + 100); opening repertoire KL with Adapter at least 5× closer to its Player than the Base is; 0/100 illegal moves in self-play.
- **Engine interface (Q13)**: Web HTTP/WebSocket only — no UCI protocol in v1. Decoding is hybrid: greedy for evaluation top-1 metric, sampling (temperature ~0.7) for live gameplay and Self-play Elo measurement.
...
4. **Action Vocabulary** — interface: `uci_string ↔ index`, `legal_mask(board) → bool[1968]`. UCI move enumeration, index assignment stable across runs, mask alignment validated against `python-chess`.
...
- `POST /move` — body `{ fen: string, adapter_id: "magnus" | "hikaru" | "firouzja" | "caruana" | "sindarov", decoding: "greedy" | "sample", temperature?: number }` → response `{ move_uci: string, policy?: { uci: string, prob: number }[] }`. Engine guarantees the returned move is legal in the given Position.
...
- Base model checkpoint: a single `.pt` file (~50 MB FP16), produced by Base Trainer.
- Adapter weights: one `.pt` file per Roster Player (~few hundred KB), produced by Adapter Trainer, namespaced by Player ID.
- Roster manifest: `src/training/roster.json` with `[{id, display_name, chess_com_username, time_control, style_note}]`.
- Evaluation reports: one JSON per Adapter with all four check results, attached to releases.
...
- **LoRA on FFN or policy head**. Adapter currently wraps attention QKVO only. Extending the adapter to FFN or the policy head is a v2 escalation if v1 evaluation underperforms.
...
- **Multiplayer / matchmaking / accounts**. v1 is a single-user-per-session experience with no persistence beyond the active game state in the browser tab.
...
- `CONTEXT-MAP.md` at the repository root and `src/training/CONTEXT.md` establish the domain vocabulary used throughout this PRD (Position, Game, Player, Roster, Base model, Adapter, Move-match accuracy, Self-play Elo). Engine and Web context CONTEXT.md files will be added once code-time reveals which terms are genuinely domain-meaningful versus implementation incidentals.
